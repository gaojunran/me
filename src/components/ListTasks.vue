...
<template>
  <div class="max-w-300">
    <div v-if="loading" text-center py10 op50>
      加载中...
    </div>

    <div v-else-if="error" text-center py10 op50>
      加载失败，请稍后重试
    </div>
    ...
    <div v-else>
      ...
      >
        ...
        >
          <span text-5em color-transparent absolute left--1rem top-0rem font-bold leading-1em text-stroke-1.5 text-stroke-hex-aaa op35 dark:op20>
            {{ project }}
          </span>
        </div>
        ...
        <div class="py-2 mt-4">
          ...
          <div v-if="getActiveTasks(tasks[project]).length > 0" class="mb-4">
            ...
            >
              <div class="flex-none mr-3 mt-1">
                <div class="w-4 h-4 rounded-full border-2 border-current op50" />
              </div>
              <div class="flex-auto">
                ...
                <div
                  ...
                  class="text-sm mt-1"
                  :class="isOverdue(task.dueDate) ? 'text-red-500 op75' : 'op50'"
                >
          ...
          <!-- Empty State -->
          <div
            v-if="getActiveTasks(tasks[project]).length === 0 && getCompletedTasks(tasks[project]).length === 0"
